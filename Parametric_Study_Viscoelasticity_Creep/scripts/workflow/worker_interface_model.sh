...
for h in "${HEIGHTS[@]}"; do
    ...
    # Standard postprocessing using modular Python script
    POSTPROCESS_SCRIPT="${SCRIPT_DIR}/postprocess_interface_model.py"
    TMP_POSTPROCESS_PY="${WORK_DIR}/tmp_postprocess_${h}_m_${m}_n_${n}.py"
    cp "$POSTPROCESS_SCRIPT" "$TMP_POSTPROCESS_PY"
    sed -i "s|{EVAL_SCRIPTS_PATH}|${BASE_DIR}/evaluation_scripts|g; s|{H}|${h}|g; s|{ANGLE}|${ANGLE}|g" "$TMP_POSTPROCESS_PY"
    if ! mamba run -n alexandros_Plot pvpython "$TMP_POSTPROCESS_PY" \
        > "${WORK_DIR}/postprocess_h_${h}_m_${m}_n_${n}.log" 2>&1; then
        echo "  ❌ Postprocessing failed for h=$h"
        exit 1
    fi
    rm -f "$TMP_POSTPROCESS_PY"
    cd "${BASE_DIR}"
done
