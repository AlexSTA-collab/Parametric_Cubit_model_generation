...
def parse_h_angle(name):
    ...
    try:
        # Try format: displacement_jump_h0.010_angle_0.0.pkl
        ...
        print(f"   ⚠️  Could not parse h/angle from filename: {name}")
        return None, None
...
def process_quantity(interface_cases_dir, full_cases_dir, quantity_prefix, quantity_name, 
                     quantity_units, error_type, out_dir, family, angle):
    """
    ...
    """
    print(f"\n{'='*70}")
    print(f"   Processing: {quantity_name} ({error_type})")
    print(f"={'='*70}")
    
    # Find reference Full model file (h=0.01) for this quantity
    full_patterns = [
        f"{quantity_prefix}_h0.01_angle_{angle}.pkl",
        f"{quantity_prefix}_h0.010_angle_{angle}.0.pkl",
        f"{quantity_prefix}_h0.010_angle_{int(angle)}.0.pkl",
    ]
    ...
    print(f"   📁 Full reference: {full_file.name}")
    
    # Find all Interface model files for this quantity
    interface_patterns = [
        f"{quantity_prefix}_*_angle_{angle}.pkl",
        f"{quantity_prefix}_*_angle_{angle}.0.pkl",
        f"{quantity_prefix}_*_angle_{int(angle)}.0.pkl",
    ]
    ...
    # Get centers - use points_bottom as proxy for element centers
    try:
        centers_full = get_coordinate_field(full_data, ["element_centers", "centers", "points_bottom"])
    except KeyError:
        # If none found, use dummy centers
        centers_full = np.zeros((len(field_full), 3))
    ...
    for int_file in sorted(interface_files):
        h_val, angle_val = parse_h_angle(int_file.name)
        ...
        # Get centers
        try:
            centers_int = get_coordinate_field(int_data, ["element_centers", "centers", "points_bottom"])
        except KeyError:
            centers_int = np.zeros((len(field_int), 3))
        ...
        print(f"   h={h_val:6.3f}: mean_abs_err={mean_abs_errors[-1]:.6e}, "
              f"mean_rel_err={mean_rel_errors[-1]:.6e}")
    ...
    print(f"\n   ✅ {quantity_name} ({error_type}) complete!")

# ==================================================
# MAIN ANALYSIS FUNCTION
# ==================================================
def run_convergence_analysis(family, angle, error_type="L1"):
    """
    ...
    """
    print("\n" + "="*70)
    print(f"  CONVERGENCE ANALYSIS: {family.upper()} - Angle {angle}°")
    print("="*70)
    ...
    family_angle_dir = base_dir / family / f"angle_{angle}"
    
    # Define case directories
    full_cases_dir = family_angle_dir / "Full_layer_models" / "cases"
    interface_cases_dir = family_angle_dir / "Interface_models" / "cases"
    out_dir = family_angle_dir / "comparison_results"
    ...
    print("\n" + "="*70)
    print(f"  ✅ CONVERGENCE ANALYSIS COMPLETE: {family.upper()} - Angle {angle}°")
...
if __name__ == "__main__":
    if len(sys.argv) < 3:
        ...
        print("\nExamples:")
        print("  python3 convergence_analysis.py stiff 0")
        print("  python3 convergence_analysis.py initial 10")
    ...
    for error_type in ALL_ERROR_TYPES:
        ...
        print(f"{'═'*70}\n")
        run_convergence_analysis(family, angle, error_type)
