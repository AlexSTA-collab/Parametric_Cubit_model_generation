...
def generate_cubit_model(cut_angle, height_of_interphase_layer, output_dir="."):
    ...
    for c in coh_vert:
        print(c)
        cubit.cmd(f"curve {c} interval 5")
    ...
    ext_surfs_interphase = exterior_surfaces2(v_cohesive)
    
    # -------------------------------------------------
    # 11. Node sets for outer faces (with interphase)
    # -------------------------------------------------
    surf_map = defaultdict(list)
    for vol, label, faces in [(bottom_vol, "bottom", ext_surfs_bot), (top_vol, "top", ext_surfs_top), 
                                                                     (v_cohesive, "interphase", ext_surfs_interphase)]:
        for s in faces:
