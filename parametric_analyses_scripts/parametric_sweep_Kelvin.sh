...
m_Js_vals=(1e-8 1e-6 1e-4 1e-2 1e0 1e2 1e4 1e6 1e8)
n_Js_vals=(1e-8 0.99)
...
TEMPLATE="${WORK_DIR}/three_body_mesh_20.0_0.1_EdelweissFE_Creep_template.inp"
...
# =======================
run_case () {
  local E_0="$1" m_Ju="$2" n_Ju="$3" m_Js="$4" n_Js="$5"
  ...
  export E_0 m_Ju n_Ju m_Js n_Js
  envsubst '${E_0} ${m_Ju} ${n_Ju} ${m_Js} ${n_Js}' \
...
# Sweeps
# =======================
# Sweep 1: E_0 x m_Ju (fix n_Ju, m_Js, n_Js)
#for E_0 in "${E_0_vals[@]}"; do
#  for m_Ju in "${m_Ju_vals[@]}"; do
    # One-at-a-time sweeps use these fixed values:
#    m_Js_fix="${m_Js_vals[0]}"
#    n_Js_fix="${n_Js_vals[0]}"
#    n_Ju_fix="${n_Ju_vals[1]}"  
#    run_case "$E_0" "$m_Ju" "$n_Ju_fix" "$m_Js_fix" "$n_Js_fix"
#  done
#done
...
for E_0 in "${E_0_vals[@]}"; do
  for m_Js in "${m_Js_vals[@]}"; do
    # One-at-a-time sweeps use these fixed values:
    n_Js_fix="${n_Js_vals[1]}"
    m_Ju_fix="${m_Ju_vals[0]}"  
    n_Ju_fix="${n_Ju_vals[0]}"
    run_case "$E_0" "$m_Ju_fix" "$n_Ju_fix" "$m_Js" "$n_Js_fix"
  done
done
