E0 = 1e-8  # Cohesive stiffness
h  = 1e-5  # Characteristic length
def write_input_file(E_0, h, output_dir="."):
    os.makedirs(output_dir, exist_ok=True)
    filename = f"three_body_model_E0_{E0:.1e}_h_{h:.1e}.inp"

    with open(filename, "w") as f:
        f.write("*node\n")
        f.write("** Our  nodes\n")
        f.write("**\n")
        f.write("*include, input=include_nodes.inp\n")

        f.write("********************************** E L E M E N T S ****************************\n")
        f.write("*element, type=Hexa8, provider=displacementelement, elset=bottom-body\n")
        f.write("*include, input=include_elset_bottom.inp\n\n")

        f.write("*element, type=IQuad4, provider=interfaceelement, elset=interface-body\n")
        f.write("*include, input=include_elset_interface.inp\n\n")

        f.write("*element, type=Hexa8, provider=displacementelement, elset=top-body\n")
        f.write("*include, input=include_elset_top.inp\n\n")

        f.write("********************************** N O D E S E T S **********************************\n")
        for direction in ["Z+", "Y+", "X+", "Z-", "Y-", "X-"]:
            f.write(f"*NSET, NSET=bottom-{direction}\n")
            f.write(f"*include, input=include_nset_bottom-{direction}.inp\n")
        for direction in ["Z-", "Y+", "X-", "Z+", "Y-", "X+"]:
            f.write(f"*NSET, NSET=top-{direction}\n")
            f.write(f"*include, input=include_nset_top-{direction}.inp\n")
    ...
    print(f"✅ Input file '{filename}' written successfully with E0={E0}, h={h}.")


if __name__ == "__main__":
    E_0 = float(sys.argv[1])
    h = float(sys.argv[2])
    write_input_file(E_0, h)
