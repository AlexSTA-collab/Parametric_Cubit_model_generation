...
BASE_WORK_DIR="${2:-$PWD}"              # 2nd argument = base work dir
WORK_DIR="${BASE_WORK_DIR}/work_E0_${E_0_input}"   # per-E0 work dir
mkdir -p "$WORK_DIR"

OUT_DIR="${WORK_DIR}/out_E0_${E_0_input}"          # per-E0 output dir
mkdir -p "$OUT_DIR"

# template fallback (looks in work dir then script dir)
TEMPLATE="${WORK_DIR}/Maxwell_Template_Relaxation_three_body_model_angle_10_n_0.1.inp"
if [[ ! -f "$TEMPLATE" ]]; then
  TEMPLATE="${SCRIPT_DIR}/Maxwell_Template_Relaxation_three_body_model_angle_10_n_0.1.inp"
fi
[[ -f "$TEMPLATE" ]] || { echo "ERROR: Template not found." >&2; exit 1; }

# find Python post-processing script
if   [[ -f "${BASE_WORK_DIR}/${POST_PY_NAME}" ]]; then POST_PY="${BASE_WORK_DIR}/${POST_PY_NAME}"
...
run_case () {
  ...
  # 3)Link mesh folder for includes
  if [[ -L "${WORK_DIR}/angle_10" || -d "${WORK_DIR}/angle_10" ]]; then
    echo "angle_10 already linked/copied in ${WORK_DIR}"
  else
    ln -s "${BASE_WORK_DIR}/angle_10" "${WORK_DIR}/angle_10"
  fi
  
  # 4) Run solver IN WORK_DIR
  ( cd "$WORK_DIR" && mamba run -n "$SOLVER_ENV" "$SOLVER_CMD" "${SOLVER_ARGS[@]}" )

  # 5) Find newest .case produced after the timestamp
  local case_file=""
  ...
  for m_Ju in "${m_Ju_vals[@]}"; do
    for n_Ju in "${n_Ju_vals[@]}"; do
      for n_Js in "${n_Js_vals[@]}"; do
        ...
        run_case "$E_M_fix" "$E_I_fix" "$E_0_fix" "$h" "$m_Js_fix" "$n_Ju" "$m_Ju" "$n_Js"
      done
    done
  done
done
