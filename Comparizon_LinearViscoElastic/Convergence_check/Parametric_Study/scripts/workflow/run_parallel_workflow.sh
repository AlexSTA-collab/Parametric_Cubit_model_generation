...
for FAMILY in "${FAMILIES[@]}"; do
    ...
    for ANGLE in "${ANGLES[@]}"; do
        for m in "${M_VALUES_TO_PROCESS[@]}"; do
            for n in "${N_VALUES_TO_PROCESS[@]}"; do
                # Each line: FAMILY ANGLE m n E_M E_I E_0
                echo "$FAMILY $ANGLE $m $n $E_M $E_I $E_0" >> "$JOBLIST"
            done
        done
    done
done
...
parallel --progress --jobs "$NUM_JOBS" --colsep ' ' \
    "$WORKER_SCRIPT" {1} {2} {3} {4} {5} {6} {7} "$BASE_DIR" "$NKELVIN" "$MINTAU" "$TIMETODAYS" "$MATERIALID" "${HEIGHTS_CLEAN[@]}" \
    :::: "$JOBLIST"

# Cleanup
rm -f "$JOBLIST"

# ================= FULL LAYER JOBLIST GENERATION =====================
# Generate job list for full layer models (family, angle, m, n, h=0.01)
JOBLIST_FULL_LAYER="${BASE_DIR}/parallel_jobs.txt"
rm -f "$JOBLIST_FULL_LAYER"
H_FULL=0.01
for FAMILY in "${FAMILIES[@]}"; do
    # Get family-specific parameters
    E_M_VAR="FAMILY_${FAMILY^^}_E_M"
    ...
    for ANGLE in "${ANGLES[@]}"; do
        for m in "${M_VALUES_TO_PROCESS[@]}"; do
            for n in "${N_VALUES_TO_PROCESS[@]}"; do
                # Each line: FAMILY ANGLE m n E_M E_I E_0
                echo "$FAMILY $ANGLE $m $n $E_M $E_I $E_0" >> "$JOBLIST_FULL_LAYER"
            done
        done
    done
done

# Run full layer models
WORKER_SCRIPT="${SCRIPT_DIR}/worker_full_layer_model.sh"
chmod +x "$WORKER_SCRIPT"
parallel --progress --jobs "$NUM_JOBS" --colsep ' ' \
    "$WORKER_SCRIPT" {1} {2} {3} {4} {5} {6} {7} "$BASE_DIR" "$NKELVIN" "$MINTAU" "$TIMETODAYS" "$MATERIALID" "$H_FULL" \
    :::: "$JOBLIST_FULL_LAYER"
...
# ============================================================================
echo "════════════════════════════════════════════════════════════════"
echo "Running Convergence Analysis for $FAMILY"
echo "════════════════════════════════════════════════════════════════"
for ANGLE in "${ANGLES[@]}"; do
for FAMILY in "${FAMILIES[@]}"; do
for M in "${M_VALUES_TO_PROCESS[@]}"; do
...
python3 "${BASE_DIR}/scripts/postprocessing/convergence_analysis.py" "$FAMILY" "$ANGLE" "$M" "$N" "L1"

if [ $? -eq 0 ]; then
    echo "✅ Convergence analysis completed for $FAMILY"
    echo ""
    echo "Results location:"
    echo "  ${FAMILY}/angle_${ANGLE}/m_${M}/n_${N}/comparison_results/"
else
...
echo ""
echo "╔═══════════════════════════════════════════════════════════════════════════╗"
echo "║   FAMILY $FAMILY, ANGLE $ANGLE, m $M, n $N COMPLETED SUCCESSFULLY         ║"
echo "╚═══════════════════════════════════════════════════════════════════════════╝"
echo ""
...
# ================================================================
# Run master parametric convergence aggregation and plotting
# ================================================================
echo ""
echo "Running master parametric convergence aggregation and plotting..."
python3 scripts/postprocessing/master_parametric_script.py
echo "Master parametric convergence plots generated in comparison_plots/"
echo ""
