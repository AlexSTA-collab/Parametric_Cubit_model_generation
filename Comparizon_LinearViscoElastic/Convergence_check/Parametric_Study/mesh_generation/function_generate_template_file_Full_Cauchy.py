...
def write_template_file(angle, height, output_dir="."):
    ...
    inp_filename = root_dir / f"LinearViscoElastic_Full_Cauchy_Template_three_body_model_angle_{angle:g}_h_{height:g}.inp"
    ...
    with open(inp_filename, "w") as f:
        ...
        f.write("*element, type=C3D8, provider=marmot, elset=bottom-body\n")
        f.write(f"*include, input={relpath('include_elset_bottom.inp')}\n\n")
        
        f.write("*element, type=C3D8, provider=marmot, elset=interface-body\n")
        f.write(f"*include, input={relpath('include_elset_interface.inp')}\n\n")
        
        f.write("*element, type=C3D8, provider=marmot, elset=top-body\n")
        ...
        f.write("\n*material, name=LinearElastic, id=linearelastic_body_1, provider=marmotmaterial\n")
        f.write("**Isotropic\n**E_M    nu\n $E_M, 0.3\n\n")

        f.write("\n*material, name=LinearElastic, id=linearelastic_body_2, provider=marmotmaterial\n")
        f.write("**Isotropic\n**E_I    nu\n $E_I, 0.3\n\n")

        f.write("\n*material, name=LinearViscoElasticWiechert, id=ViscoElasticMaterial, provider=marmotmaterial\n")
        f.write(f" **Isotropic\n**E_0, nu_0,  m, n, nKelvin, minTau, timeToDays, MaterialID\n") 
        f.write(f"$E_0, 0.3, $m, $n, 1, 1e-2, 1e0, $MaterialID\n\n")
        ...
        f.write("*section, name=section3, material=ViscoElasticMaterial, type=solid\n")
        ...
        f.write("nodeforces, name = TopF, nSet = top-Z+, field=displacement, 1=1e-2, f(t)=t\n")
        ...
        #second step keep force constant
        f.write("*step, solver=theSolver, maxInc=0.05, minInc=0.05, maxNumInc=1000, maxIter=1000, stepLength=0.1\n")
        f.write("options, category=NISTSolver, extrapolation=off\n")
        f.write("dirichlet, name = bottom, nSet = bottom-Z-, field=displacement, 1=0, 2=0, 3=0, f(t)=t\n")
